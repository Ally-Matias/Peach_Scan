...
for ( var i = 0; i < mangas.length/mangasperpage; i++){
    pagecount++;
    pagesnum += `<li class="page-item number ${i==0 ? "active" : ""}"><a class="page-link" href="#">${i}</a></li>` 
}
...
$("#grid-paginator .number").on('click', (event) => {
    console.log(parseInt(event.currentTarget.innerText) * mangasperpage,
...
$("#grid-paginator .skip-left").on('click', () => {
    ...
    if(temp > 0){
        ...
        $(`#grid-paginator .page-item`).removeClass('active')
        $("#grid-paginator .page-item").filter(function() {
